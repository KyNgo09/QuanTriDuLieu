...
# Thống kê khách hàng thường xuyên (VIP)
@khachhang_bp.route('/thong-ke-thuong-xuyen', methods=['GET'])
def thong_ke_khachhang_thuong_xuyen():
    """
    Thống kê khách hàng thường xuyên (VIP) trong N tháng gần nhất
    """
    so_thang = request.args.get('so_thang', type=int)
    if not so_thang or so_thang <= 0:
        return jsonify({"message": "Thiếu hoặc sai tham số 'so_thang'"}), 400
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.callproc('sp_ThongKeKhachHangThuongXuyen', [so_thang])
        ...
        return jsonify({
            "message": "Thống kê khách hàng thường xuyên thành công",
            "data": data
        }), 200
    except Exception as e:
        return jsonify({
            "message": "Lỗi khi thống kê khách hàng thường xuyên",
            "error": str(e)
        }), 500
    finally:
        if cursor:
